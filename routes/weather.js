...
function getClothingRecommendations(weather) {
  ...
  if (cond.includes('snow') || cond.includes('blizzard')) {
    recs.push(
      { item: 'Snow Boots',       icon: '🥾', reason: 'Grip & waterproofing for snow' },
      { item: 'Waterproof Jacket', icon: '🧥', reason: 'Repel wet snow' },
    );
    tips.push('Snow on the ground — watch your step!');
  }
...
router.get('/', async (req, res, next) => {
  try {
    ...
    if (lat && lon) {
      url = `${OWM_BASE}/weather?lat=${lat}&lon=${lon}&appid=${API_KEY}&units=metric`;
    } else if (city) {
      // Geocode first — supports city names, neighborhoods, zip codes, etc.
      const geoRes = await axios.get(
        `${OWM_GEO}/direct?q=${encodeURIComponent(city)}&limit=1&appid=${API_KEY}`
      );
      if (!geoRes.data || geoRes.data.length === 0)
        return res.status(404).json({ error: 'Location not found. Try a different city name.' });
      ({ name: geoName, country: geoCountry, lat: gLat, lon: gLon } = geoRes.data[0]);
      url = `${OWM_BASE}/weather?lat=${gLat}&lon=${gLon}&appid=${API_KEY}&units=metric`;
    } else {
    ...
    // Save to Supabase search history if user is authenticated
